...
def classify_tool(user_message: str, chat_history: list[dict], tool_history: dict, pending_cases: list[dict], loop_count: int, llm_client: LLMClient) -> dict[str, Any]:
    ...
    prompt = CLASSIFICATION_PROMPT.replace("{current_date}", current_date).replace("{pending_cases_context}", pending_context)
