...
5. หากผู้ใช้ถามหลายเรื่อง ให้พยายามเรียก Tool เท่าที่ข้อมูลครบถ้วน แต่หาก Tool ไหนต้องใช้พารามิเตอร์ที่คุณไม่ทราบค่า (เช่น `account_id` ที่ผู้ใช้ไม่ได้ระบุ)
   -> **ห้ามเดาหรือสร้างค่าสมมติขึ้นมาเองเด็ดขาด (เช่น ห้ามใส่ 'INVESTMENT_ACCOUNT_ID')**
   -> ให้ข้าม Tool นั้นไปเลย (ไม่ต้องใส่ใน `requires_tools`) เพื่อปล่อยให้บอทดึงข้อมูลส่วนที่ทำได้ไปก่อน แล้วไปถามรายละเอียดเพิ่มในขั้นตอนการตอบกลับ
...
def classify_tool(user_message: str, chat_history: list[dict], tool_history: dict, llm_client: LLMClient) -> dict[str, Any]:
    prompt = CLASSIFICATION_PROMPT
    if tool_history:
        tool_data_text = format_tool_data_for_prompt(tool_history)
        print("tool_data_text", tool_data_text)
